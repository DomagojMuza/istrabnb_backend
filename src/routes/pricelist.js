...
//     console.log(new Date(req.body.dateFrom));
    try {
            ...
            let ob = await Objects.findById(req.body.object_id).populate('pricelist');
            ob['pricelist'].push(pricelist._id);
            ob.save();
...
pricelist.delete('/api/pricelist/:id', async (req, res) => {
      ...
      try {
          ...
          let ob = await Objects.findById(item.object_id);
          console.log(ob);
          console.log(item);
          ob.pricelist = ob.pricelist.filter( pItem => pItem._id != item._id);
          ob.save();
          res.send(item);
...
function calculator(pricelist, dateFrom, dateTo) 
{
      ...
      return {total, days, period: {bookDateFrom, bookDateTo} };
}
